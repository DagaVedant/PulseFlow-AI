...
async def list_patients(
    department: Optional[str] = None, severity: Optional[str] = None, limit: int = 50
):
    """
...
async def get_patient_stats():
    ...
    for p in patients:
        ...
        bucket = (
            "critical"
            if risk >= 0.75
            else ("high" if risk >= 0.5 else ("moderate" if risk >= 0.25 else "low"))
        )
        risk_buckets[bucket] += 1
    avg_wait = sum(p.get("total_wait_time", 0) for p in patients) / max(
        1, len(patients)
    )
    return {

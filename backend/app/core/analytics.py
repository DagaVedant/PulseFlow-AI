...
class ForecastResult:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            if self.predicted_peak
            else None,
            "predicted_peak_time": round(self.predicted_peak_time, 1)
...
class HospitalForecaster:
    ...
    def _forecast_series(
        ...
        sigma = max(
            sigma, 0.01 * (max(values) - min(values) if len(values) > 1 else 0.1)
        )
        ...
        if len(forecast) > 10:
            ...
            trend_str = (
                "increasing"
                if tv > threshold
                else ("decreasing" if tv < -threshold else "stable")
            )
        else:
            trend_str = "stable"
        ...
        severity = (
            ...
            else ("warning" if peak_val >= threshold_warning else "normal")
        )
        ...
        return ForecastResult(
            ...
            predicted_peak_time=sim_time + peak_idx + 1,
    ...
    def generate_bottleneck_predictions(
        ...
        for dept_key, dept_data in depts.items():
            ...
            hist_occ = [
                ...
                if isinstance(h.get("departments"), dict)
                else occ
                for h in history[-10:]
            ] or [occ]
            ...
            if occ > 0.65 or queue > 3:
                ...
                severity = (
                    "critical" if occ > 0.85 else ("warning" if occ > 0.70 else "info")
                )
                predictions.append(
                    {
                        "department": display,
...
class OptimizationResult:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "predicted_throughput_increase": round(
                self.predicted_throughput_increase, 1
            ),
            "predicted_utilization_improvement": round(
                self.predicted_utilization_improvement, 3
            ),
...
class HospitalOptimizer:
    ...
    def optimize(self, inp: OptimizationInput) -> OptimizationResult:
        ...
        """
        return (
            self._optimize_ortools(inp)
            if ORTOOLS_AVAILABLE
            else self._optimize_scipy(inp)
        )
    ...
    def _optimize_ortools(self, inp: OptimizationInput) -> OptimizationResult:
        ...
        d_er_nurse = solver.NumVar(
            -inp.er_nurses // 2, inp.available_nurses, "er_nurse"
        )
        d_icu_doc = solver.NumVar(-inp.icu_doctors, inp.available_doctors, "icu_doc")
        d_icu_nrs = solver.NumVar(
            -inp.icu_nurses // 2, inp.available_nurses, "icu_nurse"
        )
        d_wd_doc = solver.NumVar(-inp.ward_doctors, inp.available_doctors, "wd_doc")
        d_wd_nurse = solver.NumVar(
            -inp.ward_nurses // 2, inp.available_nurses, "wd_nurse"
        )
    ...
    def _optimize_scipy(self, inp: OptimizationInput) -> OptimizationResult:
        ...
        constraints = [
            {
                "type": "ineq",
                "fun": lambda x: inp.available_doctors - (x[0] + x[2] + x[4]),
            },
            {
                "type": "ineq",
                "fun": lambda x: inp.available_nurses - (x[1] + x[3] + x[5]),
            },
        ]
        res = scipy_optimize.minimize(
            objective, [0.0] * 6, method="SLSQP", bounds=bounds, constraints=constraints
        )
        if not res.success:
            return self._fallback_result(inp)
    ...
    def _build_recommendations(
        ...
        add(
            ...
            er_nurse_delta,
            0.15 * er_l,
            "ER nurse-to-patient ratio strained",
            "high" if er_l > 0.6 else "low",
        )
        add(
            "ICU",
            "doctors",
        ...
        add(
            ...
            icu_nurse_delta,
            0.35 * icu_l,
            "ICU nurse-patient ratio compromised",
            "critical" if icu_l > 0.8 else "high",
        )
        add(
            "Ward",
    ...
    def _identify_root_causes(self, inp: OptimizationInput) -> List[str]:
        ...
        if inp.er_queue > 8:
            causes.append(
                f"ER queue backlog: {inp.er_queue} patients awaiting assessment"
            )
        if inp.icu_patients / max(1, inp.icu_beds) > 0.85:
            causes.append(
                f"ICU near capacity ({inp.icu_patients}/{inp.icu_beds}): blocking high-acuity ER transfers"
            )
        if inp.lab_queue > 10:
            causes.append(
                f"Laboratory backlog ({inp.lab_queue} samples): delaying diagnosis"
            )
        if inp.imaging_queue > 6:
            causes.append(
                f"Imaging queue ({inp.imaging_queue} patients): delaying treatment decisions"
            )
        if inp.ward_beds - inp.ward_patients < 5:
            causes.append(
                f"Ward near capacity ({inp.ward_patients}/{inp.ward_beds}): creating ICU boarding"
            )
        if inp.ct_failure:
    ...
    def _build_intervention_plan(
        ...
        for r in recs[:5]:
            plan.append(
                f"{'Add' if r.delta > 0 else 'Redeploy'} {abs(r.delta)} {r.resource_type} to {r.department} (impact: {r.impact_score:.2f})"
            )
        if inp.lab_queue > 8:
    ...
    def _detect_bottleneck(self, inp: OptimizationInput):
        ...
        """
        scores = {
            "ER": inp.er_queue / max(1, inp.er_beds)
            + inp.er_patients / max(1, inp.er_beds),
            "Laboratory": inp.lab_queue / max(1, inp.lab_analyzers),
            "Imaging": inp.imaging_queue
            / max(1, inp.ct_scanners + inp.mri_machines + 3),
        ...
        severity = (
            "critical"
            if score > 0.90
            else ("high" if score > 0.70 else ("medium" if score > 0.50 else "low"))
        )
        metrics = {
            "ER": "queue length",
...
def _ollama_chat(
    model: str, base_url: str, messages: list, max_tokens: int = 180
) -> str:
    """
...
class AICopilot:
    ...
    async def generate_intervention_narrative(self, opt: dict, state: dict) -> dict:
        ...
        recs_text = "\n".join(
            [
                f"- {r['resource_type']} in {r['department']}: {'+' if r['delta'] > 0 else ''}{r['delta']}"
                for r in recs[:5]
            ]
        )
        ...
        if narrative:
            return {
                ...
                "predicted_outcomes": {
                    ...
                    "utilization_improvement": opt.get(
                        "predicted_utilization_improvement", 0
                    ),
                },
                "model": self.model,
            }
    ...
    async def generate_shift_report(self, state: dict, history: List[dict]) -> str:
        ...
        return await self._call(prompt, 180) or self._fallback_shift_report(
            state, history
        )
    ...
    def _build_bottleneck_context(self, state: dict, opt: dict) -> str:
        ...
        m = state.get("metrics", {})
        lines = [
            f"Active: {m.get('active_patients', 0)} | Avg wait: {m.get('avg_wait_time', 0):.0f}m"
        ]
        for key, d in depts.items():
            lines.append(
                f"{d.get('display_name', key)}: {d.get('current_patients', 0)}/{d.get('capacity', 0)} beds, Q:{d.get('queue_length', 0)}"
            )
        return "\n".join(lines)

    def _fallback_bottleneck_explanation(
        self, state: dict, opt: dict, context: str
    ) -> dict:
        """
    ...
    def _fallback_intervention_narrative(self, opt: dict) -> dict:
        ...
        dept = opt.get("bottleneck_department", "ER")
        primary = next(
            (r for r in recs if r.get("urgency") in ("critical", "high")),
            recs[0] if recs else None,
        )
        action = (
            f"Move {abs(primary['delta'])} {primary['resource_type']} to {primary['department']}"
            if primary
            else "redistribute staff to match patient load"
        )
        narrative = (
    ...
    def _fallback_shift_report(self, state: dict, history: List[dict]) -> str:
        ...
        status = (
            "critical"
            if bed_util > 0.90
            else ("elevated" if bed_util > 0.75 else "stable")
        )
        icu_util = m.get("icu_utilization", 0)
        er_queue = er.get("queue_length", 0)
        priority = (
            "ICU capacity is the immediate concern"
            if icu_util > 0.85
            else f"ER queue of {er_queue} requires additional triage staff"
            if er_queue > 10
            else "maintain current staffing ratios"
        )
    ...
    def to_dict(self, now: float) -> dict:
        ...
        status = (
            ...
            if avail == 0
                ...
                "in_surgery"
                if (
                    "Surgery" in self.current_assignment
                    or "OR" in self.current_assignment
                )
                else "busy"
            )
        )
        return {
            ...
            "available_in_min": avail,
            "status": status,
            "current_assignment": self.current_assignment
            if avail > 0
            else "Available for consult",
        }
...
class FixedBottleneck:
    ...
    def to_dict(self, now: float) -> dict:
        ...
        """
        release_in = (
            max(0, round(self.release_at - now))
...
class TrackedPatient:
    ...
    def risk(self, now: float) -> float:
        ...
        return min(
            0.99,
            self.base_risk
            + min(0.02, (elapsed / max(1, self.target_window_min)) * 0.02),
        )
...
class CareCoordinator:
    ...
    def _seed(self, now: float = 0.0):
        ...
        for data in [
            ...
            },
            {
                ...
                "priority": "high",
                "notes": "Heart failure specialist pulled to ICU",
    ...
    def add_bottleneck(self, data: dict, now: float = 0.0) -> dict:
        ...
        bn = FixedBottleneck(
            bottleneck_id=bid,
            resource_name=data.get("resource_name", "Unnamed"),
    ...
    def _find_specialist(
        self, specialty: str, preferred_role: str, now: float
    ) -> Optional[Specialist]:
        """
        ...
        pool.sort(
            key=lambda sp: (
                sp._available_in(now),
                ranked.index(sp.role) if sp.role in ranked else len(ranked),
            )
        )
        return pool[0]
    ...
    def _blocking_bottleneck(
        self, specialist: Optional[Specialist], now: float
    ) -> Optional[FixedBottleneck]:
        """
    ...
    def _specialist_dict(self, sp: Specialist, now: float) -> dict:
        ...
        if bn:
            bd = bn.to_dict(now)
            d["available_in_min"] = (
                bd["release_in_min"] if bd["release_in_min"] is not None else 999
            )
            d["status"] = (
                "in_surgery"
                if ("surg" in bn.status.lower() or " or" in bn.status.lower())
                else "busy"
            )
    ...
    def _build_recommendation(self, tp, sp, blocker, wait, risk, now) -> dict:
        ...
        if wait > tp.target_window_min:
            reasons.append(
                f"{tp.condition} exceeds {tp.target_window_min}-min target (waited {round(wait)}m)"
            )
        else:
            reasons.append(
                f"{tp.condition} within {tp.target_window_min}-min target ({round(wait)}m elapsed)"
            )
        alternative = None
        if sp:
            ...
            if blocker:
                ...
                until = bd["release_label"] or (
                    f"{bd['release_in_min']}m"
                    if bd["release_in_min"] is not None
                    else "later"
                )
                alt = self._find_alternative(tp, sp, now)
                alternative = alt.name if alt else None
                reasons.append(
                    f"{blocker.resource_name} is a fixed constraint until {until}"
                )
                if alt:
                    ...
                    reasons.append(
                        ...
                        + (
                            "available now"
                            if alt_eta == 0
                            else f"available in {alt_eta}m"
                        )
                    )
            else:
                reasons.append(
                    f"{sp.role} {sp.name} is "
                    + ("available now" if eta == 0 else f"available in {eta}m")
                )
        else:
            reasons.append(f"No {tp.awaiting_specialty} specialist on roster")
        deterioration = round(risk * 19)
        throughput = {"critical": 9, "high": 7, "moderate": 4, "low": 2}.get(
            tp.priority, 4
        )
